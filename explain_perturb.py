...
def shap(model, X, ys=None):
    import shap
    # explain predictions of the models on four images
    if tf.is_tensor(X):
        X = X.numpy()
    e = shap.GradientExplainer(model, X)
    # ...or pass tensors directly
    # e = shap.DeepExplainer((models.layers[0].input, models.layers[-1].output), background)
...
def adv_train(model, inputs, outputs, loss, learning_rate, alpha,
    ...
    with tf.GradientTape() as tape:
     y_pred = models.predict(x, variables)
     loss = models.compute_loss(y_pred, y)
    ...
    with tf.GradientTape(persistent=True) as t:
        ...
        explanation = []
        for i in range(len(sensitive_feature_ids)):
            explanation.append(t.gradient(performance_loss, [inputs])[0][:, sensitive_feature_ids[i]])

        # explanation1 = t.gradient(performance_loss, [inputs])[0][:, sensitive_feature_id1]
        # explanation2 = t.gradient(performance_loss, [inputs])[0][:, sensitive_feature_id2]
        ...
        explanation_loss = []
        for i in range(len(sensitive_feature_ids)):
            explanation_loss.append(tf.norm(explanation[i], norm))
        # explanation_loss1 = tf.norm(explanation1, norm)
        # explanation_loss2 = tf.norm(explanation2, norm)
        if normalise:
            for i in range(len(sensitive_feature_ids)):
                explanation_loss[i] = tf.compat.v2.math.divide(explanation_loss[i], tf.cast(inputs.shape[0], "float32"))
            # explanation_loss1 = tf.compat.v2.math.divide(explanation_loss1, tf.cast(inputs.shape[0], "float32"))
            # explanation_loss2 = tf.compat.v2.math.divide(explanation_loss2, tf.cast(inputs.shape[0], "float32"))
        # explanation_loss = explanation_loss1 + explanation_loss2
        explanation_loss = sum(explanation_loss)
        # temp = sum(explanation_loss)
        ...
        # total_loss = performance_loss + alphas[0] * explanation_loss
    ...
    #     acc = models.evaluate(inputs,outputs)
    history = {}
...
# WRAPPER
# -----------------------------------------------------------------------------
def wrap_adv_train(X_test, inputs, Y_test, outputs, model, z_idxs, alpha,
                   n_epochs=40,
                   lr=10e-7,
                   normalise=True,
                   norm=1,
    ...
    explanation_loss = 0
    for i in range(len(sensitive_feature_ids)):
        explanation_loss = explanation_loss + tf.norm(explanation[i], norm)
    # explanation_loss1 = tf.norm(explanation1, norm)
    # explanation_loss2 = tf.norm(explanation2, norm)
    # explanation_loss = explanation_loss1 + explanation_loss2
    ...
    # these were the params to get the kick-ass models?
